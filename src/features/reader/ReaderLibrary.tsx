  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-6 py-8">
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-muted-foreground">Import a file and listen without editing the script.</p>
        <Button
          disabled={busy}
          onClick={async () => {
            ...
            try {
              ...
              await refresh();
              await navigate({ to: "/reader/$docId", params: { docId: doc.id } });
            } catch (err) {
      ...
      <DocumentList documents={documents} mode="listen" empty="No documents yet. Import something to listen." />
    </div>
  );
}
